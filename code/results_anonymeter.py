...
risk_dpart_independent['technique'] = 'DPART'
# %%
results = []
results = pd.concat([risk_ppt, risk_resampling, risk_deeplearning, risk_privateSMOTE, risk_privateSMOTE_force,
                     risk_privateSMOTE_laplace, risk_privateSMOTE_force_laplace, risk_dpart_independent])
...
# %%
order = ['PPT', 'RUS', 'SMOTE', 'BorderlineSMOTE', 'Copula GAN', 'TVAE', 'CTGAN', 'DPART', 'PrivateSMOTE', 'PrivateSMOTE Force', r'$\epsilon$-PrivateSMOTE', r'$\epsilon$-PrivateSMOTE Force']
# %%  BETTER IN PRIVACY
...
priv_results['ds_complete'] = priv_results['ds_complete'].apply(lambda x: re.sub(r'_qi[0-9]','', x) if (('TVAE' in x) or ('CTGAN' in x) or ('copulaGAN' in x) or ('dpart' in x) or ('smote' in x) or ('under' in x)) else x)
...
order_performance_bestprivsmote = ['PPT', 'RUS', 'SMOTE', 'BorderlineSMOTE', 'Copula GAN', 'TVAE', 'CTGAN', 'DPART', r'$\epsilon$-PrivateSMOTE Force']
...
# %%
order_privsmote_bestperformance = ['PPT', 'RUS', 'SMOTE', 'BorderlineSMOTE', 'Copula GAN', 'TVAE', 'CTGAN', 'DPART', r'$\epsilon$-PrivateSMOTE Force']

privsmote_bestperformance = pred_.loc[(pred_.technique!='privateSMOTE') &\
                                                   (pred_.technique!='privateSMOTE_force')\
                                                  & (pred_.technique!='privateSMOTE_laplace')]
# %%
sns.set_style("darkgrid")
