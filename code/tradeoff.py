...
sns.boxplot(ax=axes[0], data=privsmote,
    x=r'$\epsilon$', y='roc_auc_perdif', order=order_eps, **PROPS)
sns.boxplot(ax=axes[1], data=privsmote,
    x=r'$\epsilon$', y='value', order=order_eps, **PROPS)
...
# %%
privsmote_max = privsmote.loc[privsmote.groupby(['ds', r'$\epsilon$'])['roc_auc_perdif'].idxmax()]
sns.set(style='darkgrid')
...
sns.scatterplot(x="roc_auc_perdif",
                    ...
                    data=city)
# %%
city[r'$\epsilon$'] = np.nan
for idx, file in enumerate(city.ds_complete):
    city[r'$\epsilon$'][idx] = str(list(map(float, re.findall(r'\d+\.\d+', file.split('_')[4])))[0])
