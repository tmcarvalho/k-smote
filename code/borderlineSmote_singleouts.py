...
def interpolation_singleouts(original_folder, file):
    ...
    """
    output_interpolation_folder = '../output/oversampled/borderlineSmote_singleouts/'
        ...
        for nn in knn:
            for ratio in ratios:
                ...
                try:
                    ...
                    oversample.to_csv(
                        f'{output_interpolation_folder}{sep}ds{file.split(".csv")[0]}_bordersmote_QI{idx}_knn{nn}_per{ratio}.csv',
                        index=False)    
...
class Smote:
    ...
    def _in_danger_noise(self, samples, target_class, y, neighbors):
        ...
        """
        x = neighbors.kneighbors(samples, return_distance=False)[:, 1:]
    ...
    def over_sampling(self):
        ...
        danger_index = self._in_danger_noise(self.samples, class_sample, y,
                                                 neighbors)
...
def interpolation_singleouts_scratch(original_folder, file):
    ...
    output_interpolation_folder = '../output/oversampled/borderlineSmote_singleouts_scratch/'
    data = pd.read_csv(f'{original_folder}/{file}')
...
datafile = '../original/2.csv'
# df = loaddata(datafile)
df = pd.read_csv(datafile)

print(Counter(df['def']))
interpolation_singleouts_scratch(df)